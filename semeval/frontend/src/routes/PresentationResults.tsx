  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-surface-950 text-white flex flex-col">
        <Header />
        <main className="flex-1 flex items-center justify-center text-white/40 text-sm animate-pulse">
          Loading results…
        </main>
      </div>
    );
      ...
      <div className="min-h-screen bg-surface-950 text-white flex flex-col">
        ...
        <main className="flex-1 mx-auto w-full max-w-xl px-4 py-16 text-center flex flex-col items-center justify-center">
          <div className="glass-card p-10 flex flex-col items-center gap-4 w-full">
            ...
            <h2 className="text-2xl font-bold text-white">No Score Yet</h2>
            <p className="text-sm text-white/60">{error || "This presentation hasn't been scored."}</p>
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-8">
          <div className="flex flex-col gap-1 glass-card p-6">
            ...
            <p className="text-sm text-white/70 mt-0.5">{presentation?.topic}</p>
            {presentation && presentation.members.length > 0 && (
              <p className="text-xs text-white/50 mt-1">{presentation.members.join(", ")}</p>
            )}
          </div>
                  ...
                  {score.dimension_scores.map((ds) => (
                    <div key={ds.dimension} className="flex flex-col gap-1 border-b border-white/5 pb-2">
                      <div className="flex justify-between items-center gap-2 text-xs font-medium">
                        ...
                        <span className="shrink-0 font-mono text-brand-300 font-bold">
              ...
              {presentation && (
                <div className="glass-card p-4 flex flex-col gap-2">
                  <button
                    ...
                    className="text-xs font-bold uppercase tracking-wider text-brand-300 text-left"
                  >
                    {showTranscript ? "▾" : "▸"} Full Transcript
                  </button>
                  {showTranscript && (
                    <p className="text-xs text-white/70 leading-relaxed max-h-64 overflow-y-auto whitespace-pre-wrap">
              ...
              <div className="glass-card p-6 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                  {score.positives.length === 0 ? (
                    <p className="text-xs text-white/40 italic">No specific positives identified.</p>
                  ) : (
              ...
              <div className="glass-card p-6 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                  {score.negatives.length === 0 ? (
                    <p className="text-xs text-white/40 italic">No specific negatives identified.</p>
                  ) : (
